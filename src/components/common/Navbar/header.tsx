...
const Header = ({ children }: HeaderProps): JSX.Element => {
  ...
  const { isLoggedIn, user, account, role, inbox, logOut } = useUsersContext()
  ...
  const Section = (props) => {
    ...
    const hbg = useColorModeValue('gray.100', 'inerit')
    const tcl = useColorModeValue('gray.900', 'brand.200')
    const dcl = useColorModeValue('gray.500', 'white')
    return (
  ...
  const Features = (props) => {
    ...
    const hbgh = useColorModeValue('gray.300', 'pink.500')
          ...
          >
            ...
            >
              <Text>
                ...
                debates on the best way to run things, it’s all happening on DAO
                Proposals. So come join us and see what all the fuss is about!
              </Text>
            </Section>
          </Link>
