...
export class TextileInstance {
    ...
    public async uploadCollectionMetadata(collection: Collection) {
        console.log("uploadCollectionMetadata func ");
        ...
        const uploadName = `${now}_${collection.name}`;
        const metadataLocation = `collections/${collection.name}`;

        const blob = new Blob([JSON.stringify(collection)], {
            type: "application/json",
        });
        const file = new File([blob], collection.name);
    ...
    public async addCollectionToUserBucket(collection: Collection) {
        await this.client.create(this.threadID, "collections", [collection]);
    }

    public async getUserCollections(): Promise<Collection[]> {
        let collections;
        try {
            collections = await this.client.find(
                this.threadID,
                "collections",
                {}
            );
